...
## Langkah - langkah Deteksi Garis Tepi
### 1. Membaca Citra
Membaca citra adalah langkah pertama dalam pemrosesan citra digital. Ini melibatkan memuat citra dari sumber penyimpanan (seperti file gambar) ke dalam memori komputer sehingga dapat diproses lebih lanjut.

- **Pustaka Pemrosesan Citra:** Ada beberapa pustaka yang digunakan untuk membaca citra, seperti OpenCV dan skimage. Pustaka ini mendukung berbagai format gambar seperti JPEG, PNG, TIFF, dll.
- **Tujuan:** Memastikan bahwa citra diimpor dengan benar sehingga dapat diproses lebih lanjut.
### 2. Konversi ke Grayscale
Mengonversi citra berwarna menjadi grayscale adalah langkah penting sebelum melakukan deteksi tepi.

- **Apa itu Citra Grayscale?:** Citra grayscale hanya memiliki satu kanal warna, dibandingkan dengan citra berwarna yang biasanya memiliki tiga kanal (merah, hijau, biru). Grayscale merepresentasikan intensitas cahaya, membuatnya lebih sederhana untuk dianalisis.
- **Mengapa Perlu Konversi?:** Deteksi tepi lebih mudah dilakukan pada citra grayscale karena hanya perlu mempertimbangkan perubahan intensitas cahaya, bukan perubahan warna. Ini menyederhanakan komputasi dan meningkatkan akurasi.
- **Proses Konversi:** Pustaka pemrosesan citra menggunakan fungsi untuk mengubah citra berwarna menjadi grayscale dengan menghitung rata-rata atau menggabungkan kanal warna berdasarkan bobot tertentu.
### 3. Penghapusan Noise
Noise dalam citra adalah variasi acak dari intensitas atau warna yang tidak diinginkan yang dapat mengganggu proses deteksi tepi. Mengurangi noise sangat penting untuk mendapatkan hasil deteksi tepi yang lebih akurat.

- **Apa itu Noise?:** Noise dapat berasal dari berbagai sumber, termasuk sensor kamera, kondisi pencahayaan yang buruk, atau proses pemampatan citra. Noise muncul sebagai titik-titik acak atau variasi yang tidak diinginkan dalam citra.
- **Metode Penghapusan Noise:** Salah satu metode yang umum digunakan untuk mengurangi noise adalah Gaussian blur. Gaussian blur menghaluskan citra dengan menerapkan filter Gaussian, yang menyebarkan intensitas piksel di sekitar setiap piksel, mengurangi perbedaan tajam yang disebabkan oleh noise.
- **Mengapa Gaussian Blur?:** Filter Gaussian menyaring noise sambil mempertahankan tepi yang lebih signifikan, membuatnya ideal untuk persiapan deteksi tepi.
